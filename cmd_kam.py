...
class KAM(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.command(aliases=['LS geo tb-hez KAM ellenőrző'])
    @commands.has_any_role(global_settings.Role1, global_settings.Role2)  # User need this role to run command (can have multiple)
    async def kam(self, ctx, raw_allycode):
        """LS geo tb-hez KAM készenlét ellenőrző teljes guildre
        Végigelemzi a teljes guildet KAM készenlétre.
