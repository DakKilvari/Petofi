...
class Bakery(commands.Cog):
            ...
            if myresult != None:
                ...
                if tipus == "zacc":
                    ...
                    if n >= 3:
                        ...
                        print(mycursor.rowcount, "record(s) affected")
                        await ctx.send(
                            "<@" + DiscordID + ">" + " beváltott három 🍪-t egy ☕ kiváltására, így van neki " + str(t))
                    else:
                        await ctx.message.add_reaction("❌")
    ...
    async def mennyi(self, ctx, userid: str):
        ...
        if DiscordID != "000000000":
            ...
            if userid == "sör":
                await ctx.message.add_reaction("✅")
                await ctx.send("Végtelen ingyen sör.")
            else:
                await ctx.message.add_reaction("❌")
                await ctx.send(DiscordID + " nincs az adatbázisban.")
    ...
    @commands.has_any_role(global_settings.Role1,
                           global_settings.Role2,
                           global_settings.Role4)  # User need this role to run command (can have multiple)
